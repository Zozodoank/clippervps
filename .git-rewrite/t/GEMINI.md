...
* **Public IP Server:** `REDACTED_IP`
* **SSH Port:** `REDACTED_PORT` (Diteruskan ke port internal `22`)
...
2. Untuk menerapkan perubahan di server VPS:
   ...
   * Atau jalankan manual via SSH:
     ```bash
     ssh -p REDACTED_PORT ubuntu@REDACTED_IP "cd ~/clipperVPS && git pull && pm2 restart all"
     ```
